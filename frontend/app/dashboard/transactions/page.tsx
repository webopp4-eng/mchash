  ...
  return (
    <div className="mc-page">
      <section className="mc-page-header">
        ...
        <div className="rounded-2xl bg-cmblue-50 px-3 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-sm font-extrabold text-cmblue-700 ring-1 ring-cmblue-100">
      ...
      <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1">
        {filters.map((filter) => (
          <button
            ...
            className={`shrink-0 rounded-lg sm:rounded-xl px-2.5 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold ${
              activeFilter === filter
        ...
        {visible.length > 0 ? (
          <div className="space-y-2">
            {visible.map((tx: any) => {
              const Icon = tx.icon;
              return (
                <div key={tx.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 rounded-2xl border border-sky-100 bg-sky-50/50 p-2.5 sm:p-3">
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    <span className={`mc-stat-icon ${tx.color} shrink-0`}>
                      <Icon className="h-4 w-4" />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] sm:text-xs font-bold text-slate-950 truncate">{tx.label}</p>
                      <p className="text-[8px] sm:text-[10px] text-slate-500">{new Date(tx.createdAt).toLocaleString()}</p>
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-3 text-xs sm:text-sm font-semibold text-slate-500">No transactions found</p>
          </div>
        )}
      </section>
    </div>
