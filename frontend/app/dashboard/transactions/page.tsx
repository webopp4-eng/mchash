...
export default function TransactionsPage() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'deposit': return { icon: FaArrowDown, color: 'bg-emerald-500/20 text-emerald-400' };
      case 'withdrawal': return { icon: FaArrowUp, color: 'bg-rose-500/20 text-rose-400' };
      case 'purchase': return { icon: FaWallet, color: 'bg-cmblue-500/20 text-cmblue-400' };
      case 'referral': return { icon: FaUsers, color: 'bg-purple-500/20 text-purple-400' };
      default: return { icon: FaBolt, color: 'bg-amber-500/20 text-amber-400' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Complete transaction history</p>
      </div>
      ...
      <div className="rounded-[24px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
        {transactions.length > 0 ? (
                ...
                <div key={tx.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {tx.txHash && (
                        <p className="mt-0.5 text-[9px] text-slate-600">Hash: {tx.txHash.slice(0, 12)}...</p>
                      )}
                    </div>
                  </div>
          ...
          <div className="py-10 text-center">
            <FaExchangeAlt className="mx-auto h-10 w-10 text-slate-600" />
