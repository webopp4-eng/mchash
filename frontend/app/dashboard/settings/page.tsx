  ...
  const Switch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 ${
        checked ? 'bg-cmblue-600' : 'bg-slate-700'
      }`}
    >
      <span
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[24px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-semibold text-cmblue-300">Profile</h2>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <p className="mt-1 text-sm font-semibold">{user ? shortenAddress(user.walletAddress, 8) : ''}</p>
            <p className="mt-0.5 text-[10px] capitalize text-slate-500">{user?.walletType || 'Wallet'} • {user?.chain || ''}</p>
          </div>
          <button
            onClick={saveUsername}
            className="rounded-xl bg-cmblue-600 px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-cmblue-500"
          >
            Save Profile
          </button>
        </div>
      ...
      <div className="rounded-[24px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-semibold text-cmblue-300">Notifications</h2>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
            <div>
              <p className="text-xs font-semibold">Mining Updates</p>
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
            <div>
              <p className="text-xs font-semibold">Deposit Alerts</p>
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
            <div>
              <p className="text-xs font-semibold">Withdrawal Alerts</p>
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3">
            <div>
              <p className="text-xs font-semibold">Marketing Emails</p>
      ...
      <div className="rounded-[24px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
        <div className="mb-4 flex items-center gap-2.5">
          <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-emerald-500/20 text-emerald-400">
            <FaShieldAlt className="h-3.5 w-3.5" />
          </span>
          <div>
            <h2 className="text-sm font-semibold text-cmblue-300">Security</h2>
        ...
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
          <div className="flex items-center gap-2">
            <FaWallet className="h-4 w-4 text-emerald-400" />
            <p className="text-xs font-semibold text-emerald-300">Wallet Signature Authentication</p>
          </div>
