...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-[24px] border border-slate-200/80 bg-white p-6 shadow-card">
        ...
        <div className="relative flex flex-col items-center gap-4 sm:flex-row sm:items-center">
          <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-cmblue-600 to-cmblue-500 text-white shadow-blue-glow">
            <FaUserCircle className="h-10 w-10" />
          </div>
          <div className="text-center sm:text-left">
            <h1 className="text-xl font-bold text-slate-900">{user?.username || 'Miner'}</h1>
            <p className="mt-1 text-xs text-slate-500">{user ? shortenAddress(user.walletAddress, 8) : ''}</p>
            <div className="mt-2 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-semibold text-emerald-700">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                Active
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full bg-cmblue-50 px-3 py-1 text-[10px] font-semibold text-cmblue-700">
        ...
        <div className="relative mt-6 grid grid-cols-3 gap-3">
          <div className="rounded-2xl border border-slate-200/80 bg-slate-50 p-3 text-center">
            <p className="text-lg font-bold text-cmblue-600">${Number(user?.platformBalance || 0).toFixed(2)}</p>
        ...
        <div className="grid gap-2">
          {secondaryItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="group flex items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white p-3 transition-all hover:border-cmblue-300 hover:bg-cmblue-50/30 hover:shadow-sm"
            >
              ...
              <FaChevronRight className="h-3.5 w-3.5 text-slate-400 transition-transform group-hover:translate-x-0.5" />
            </Link>
          ))}
        </div>
      </div>
